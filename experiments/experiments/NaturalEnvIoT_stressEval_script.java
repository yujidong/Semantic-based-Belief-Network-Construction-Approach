...
/**
 * Script for running NaturalEnvironmentIoT stress experiment
 * simulating the testbed of 1 location
 * for different reading intervals
 * for each reading interval we vary the period from 1 to 360 months
 *  
 *  Each experiment is run for 30 runs to calculate the average of ActualSimulationTime and UsedMemory. 
 */

public class NaturalEnvIoT_stressEval_script {
	/**
	...
	public static void main(String[] args) {
		...
		int total_exp_no_months = 36;		//variable exp_no_months is used for varying the number of months
		...
		try {
			...
			String outputFileName = "NaturalEnvIoT_stressEval";
			...
			Log.printLine("for each scenario we vary the period from 1 to " + total_exp_no_months + " months");

			//variable scenario is used to represent the number of scenarios
			for (int i=0; i < ExperimentsConfigurations.READING_INTERVAL.length; i++) {	//reading intervals
				Log.printLine("Scenario with reading every " + ExperimentsConfigurations.READING_INTERVAL[i]/60/60 + " hours");

				//variable exp_no_months is used for varying the number of months for each experiment (1-360)
				for (int exp_no_months = 1; exp_no_months <= total_exp_no_months; exp_no_months++) {
					Log.printLine("Starting stress testing experiment for number of months:" + exp_no_months);
