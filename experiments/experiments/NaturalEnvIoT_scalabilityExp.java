...
/**
 * NaturalEnvironmentIoT scalability experiment
 * running the simulation for 36 months 
 * for the 3 different scenarios (dry, rainy, flood)
 * for each scenario we vary the number of locations from 1 to 300 locations 
 *  
 *  Each experiment is run for 30 runs to calculate the average of ActualSimulationTime and UsedMemory. 
 */

public class NaturalEnvIoT_scalabilityExp {
	/**
	...
	public static void main(String[] args) {
		...
		try {
			...
			OutputStream output = new FileOutputStream(resultsFolder+ "NaturalEnvIoT_scalabilityExp.txt");
			...
			XSSFSheet spreadsheet = workbook.createSheet("NaturalEnvIoT_scalabilityExp");  
			...
			h_cellB.setCellValue("No_of_Locations");
			h_cellC.setCellValue("Exp_run");
			h_cellD.setCellValue("Simulation_Time_(ms)");
			h_cellE.setCellValue("Used_Memory_(MB)");

			//Write the workbook in file system
			FileOutputStream fileOut = new FileOutputStream(new File(resultsFolder + "NaturalEnvIoT_scalabilityExp.xlsx"));
			workbook.write(fileOut);
			fileOut.close();

			//simulation parameters
			int no_months = 12;
			int no_locations = 300;
			int no_runs = 10;
			
			Log.printLine("Starting NaturalEnvironmentIoT scalability experiment...");
			Log.printLine("running the simulation for " + no_months + " months ");
			...
			//variable scenario is used to represent the number of scenarios
			for (int i=0; i < ExperimentsConfigurations.READING_INTERVAL.length; i++) {	//scenarios
				Log.printLine("Scenario with reading every " + ExperimentsConfigurations.READING_INTERVAL[i]/60/60 + " hours");

				ExperimentsConfigurations.EXP_NO_OF_DAYS = no_months*30;	//x months *30days

				//variable exp_no_locations is used for varying the number of locations for each experiment (1-300)
				for (int exp_no_locations = 1; exp_no_locations <= no_locations; exp_no_locations++) {
					Log.printLine("Starting stress testing experiment for number of locations:" + exp_no_locations);
					
					//variable exp_run is used to run each experiment for 30 times
					for (int exp_run = 1; exp_run <= no_runs; exp_run++) {
						...
						cloudletList = Workload.generateWorkloadRuntime(
								workloadFolder + "iot//" + "iot_workload_minimal.txt", ExperimentsConfigurations.WORKLOAD_INTERVAL[i],
						...
						for (int j=1; j <= exp_no_locations; j++) {
							...
							createTestbed(datacenter0, configurations.ExperimentsConfigurations.READING_INTERVAL[i], datasetsFolder);
						}
						...
						Log.printLine();
						long actualSimTime = Duration.between(startTime,finishTime).getNano()/1000000;
						...
						InputStream fileIn = new FileInputStream(resultsFolder + "NaturalEnvIoT_scalabilityExp.xlsx");
						...
						cell1.setCellValue(ExperimentsConfigurations.READING_INTERVAL[i]/60/60);
						cell2.setCellValue(exp_no_locations);
						cell3.setCellValue(exp_run);
						cell4.setCellValue(actualSimTime);
						cell5.setCellValue(usedMemory);
						
						FileOutputStream out = new FileOutputStream(resultsFolder + "NaturalEnvIoT_scalabilityExp.xlsx");
						wb.write(out);
						out.close();
					} //end 30 runs
				} //end number of locations
			} //end reading intervals
