...
/**
 * Script for running NaturalEnvironmentIoT scalability experiment
 * running the simulation for x months
 * for different reading intervals
...
public class NaturalEnvIoT_scalabilityEval_script {
	/**
	...
	public static void main(String[] args) {
		...
		int total_exp_no_locations = 300;		
		...
		try {
			...
			String outputFileName = "NaturalEnvIoT_scalabilityEval";
			...
			Log.printLine("running the simulation for " + exp_no_months + " months ");
			Log.printLine("for different reading intervals");
			...
			for (int i=0; i < ExperimentsConfigurations.READING_INTERVAL.length; i++) {	//scenarios
				...
				for (int exp_no_locations = 1; exp_no_locations <= total_exp_no_locations; exp_no_locations++) {
					Log.printLine("Starting scalability testing experiment for number of locations:" + exp_no_locations);
					
					ExperimentsConfigurations.EXP_NO_OF_DAYS = exp_no_months*30;	//x months *30days
