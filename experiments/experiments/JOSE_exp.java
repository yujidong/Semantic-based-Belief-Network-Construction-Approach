 ...
 * each testbed has 1000 sensors of each type
...
public class JOSE_exp {
	...
	public static void main(String[] args) {
		...
		double exp_readingInterval = ExperimentsConfigurations.READING_INTERVAL[2];			//variable scenario is used to represent the reading interval
		int exp_workloadInterval = ExperimentsConfigurations.WORKLOAD_INTERVAL[2];
		int exp_no_months = 12;		//variable exp_no_months is used for varying the number of months
		int exp_no_locations = 10;	//used in this experiment for the sets of sensors in each region, each set of sensors contains 10 sensors of each type 	
		...
		try {
			...
			String outputFileName = "JOSE" + exp_no_months;
			...
			Log.printLine("Starting JOSE experiment...");
			Log.printLine("simulating the testbed of " + exp_no_locations + " location(s)");
			Log.printLine("each location has 100 sensors");
			...
			for (int run = 1; run <= exp_no_runs; run++) {
				...
				//create Hosts for 5 locations
				List<AdvHost> hostList = new ArrayList<AdvHost>();
				hostList = Setup.createAdvHostList(450, 3, 2);  //2 = VmSchedulerTimeShared
				//create Datacenters
				IoTDatacenter datacenter = Setup.createIoTDatacenter("Datacenter_0", hostList);
				...
				GeographicRegion regionTokyo = new GeographicRegion("Tokyo", 100.00);
				GeographicRegion regionOsaka = new GeographicRegion("Osaka", 100.00);
				GeographicRegion regionKyoto = new GeographicRegion("Kyoto", 100.00);
				GeographicRegion regionYokosuka = new GeographicRegion("Yokosuka", 100.00);
				GeographicRegion regionKanazawa = new GeographicRegion("Kanazawa", 100.00);

				for (int j=1; j <= exp_no_locations; j++) {
					//create one set of IoT sensors
					createTestbed(datacenter, exp_readingInterval, datasetsFolder);
				}
	...
	private static void createTestbed(Datacenter datacenter, double readingInterval, String datasetsFolder) {
		...
		for (int s=1; s<=10; s++) {
			SensorNode humiditySensor = new SensorNode(
					"HumiditySensor"+s, 
					new Location(400+s*100, 400+s*100, 0), 
					IoTNodeType.SENSOR, 
					new NetConnection("conn_shortRadio"+s, new NetConnectionType(), 100.00),
					new IoTNodePower(IoTNodePowerType.BATTERY, false, true, false, 100, 0.1, 10.00),
					"LinkNode", 
					SensorType.AIR_Humidity_SENSOR, 
					readingInterval, 
					datasetsFolder+"ukcp09_humidity.csv");
		}
		for (int s=1; s<=10; s++) {
			SensorNode airPressureSensor = new SensorNode(
					"AirPressureSensor"+s, 
					new Location(400+s*100, 400+s*100, 0), 
					IoTNodeType.SENSOR, 
					new NetConnection("conn_shortRadio"+s, new NetConnectionType(), 100.00),
					new IoTNodePower(IoTNodePowerType.BATTERY, false, true, false, 100, 0.1, 10.00),
					"LinkNode", 
					SensorType.AIR_Pressure_SENSOR, 
					readingInterval, 
					datasetsFolder+"ukcp09_vapour-pressure.csv");
		}
		for (int s=1; s<=10; s++) {
			SensorNode windDirectionSensor = new SensorNode(
					"WindSpeedSensor"+s, 
					new Location(400+s*100, 400+s*100, 0), 
					IoTNodeType.SENSOR, 
					new NetConnection("conn_shortRadio"+s, new NetConnectionType(), 100.00),
					new IoTNodePower(IoTNodePowerType.BATTERY, false, true, false, 100, 0.1, 10.00),
					"LinkNode", 
					SensorType.AIR_WindSpeed_SENSOR,
					readingInterval, 
					datasetsFolder+"ukcp09_wind-speed.csv");
		}
		for (int s=1; s<=10; s++) {
